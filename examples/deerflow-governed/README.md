...
This is an example, not a production package.

In scope:

- tool-call pre-execution authorization;
- `tool_name` and selected `tool_input` fields;
- AGT policy evaluation;
- AGT audit logging;
- local DeerFlow configuration through `guardrails.provider`.

Out of scope:

- DeerFlow source changes;
- sidecar service;
- PyPI packaging;
- rogue detection;
- model-output governance;
- skill installation governance;
- output quality gates;
- full handoff governance;
- rogue detection;
- advanced Merkle proof/tamper demos;
- dependency on populated `thread_id` or `is_subagent`.

## Future Extraction Path

The example keeps the important pieces separate enough to extract later:

```text
AGTGuardrailProvider
...
A future in-process package can reuse those pieces directly. A future sidecar
could keep the same normalization and decision model, with the DeerFlow provider
becoming an HTTP client and AGT policy/audit running in a separate process.
